class LanguageEn {
  static String get startedDiscover => "If you’re on a budget or\nsimply looking";

  static String get starttradingyourmoney =>
      "Cryptoassets are a highly volatile unregulated\ninvestment product.";

  static String get skip => "Skip";

  static String get bestappto => "Best App to Buy\nCryptocurrency";

  static String get howeverthe => "However, the main issue\nwe havet";

  static String get nextup => "Next up we have Coinbase\nwhich is potentially one";

  static String get next => "Next";

  static String get themosttrusted =>
      "The Most Trusted &\nSecure Crypto Wallet";

  static String get trackmorethan =>
      "Track more than 100+ crypto coins unders our\nuser-frendly, safe and secure platform";

  static String get getstarted => "Get Started";

  static String get signup => "Sign Up";

  static String get ittakesaminute =>
      "It only takes a minute to create your account";

  static String get fanme => "First";

  static String get lname => "Last";

  static String get emailadress => "Email address";

  static String get password => "Password";

  static String get iagreetothe => "I agree the Cryptoline";

  static String get termsofservices => "Terms of Service ";

  static String get and => "and";

  static String get privacypolicy => "Privacy Policy";

  static String get oR => "OR";

  static String get continuewithgoogle => "Continue with Google";

  static String get alreadyregistered => "Already registered?";

  static String get signin => "Sign In";

  static String get welcome => "Welcome back!";

  static String get youhavebeenmissed => "You have been missed";

  static String get forgotpassword => "Forgot Password";

  static String get setupsecondstep => "Set up 2-step verification";

  static String get donthaveanaccount => "Don’t have an account?";

  static String get enteryourphonenumber =>
      "Enter your phone number so we can send you the\nverification code";

  static String get continuee => "Continue";

  static String get enterverification => "Enter Verification Code";

  static String get enterfourdigitnumber =>
      "Enter 4-digit code the we just send to your\nphone number +62 1234567890";

  static String get resetcode => "Resend Code";

  static String get verify => "Verify";

  static String get youreverified => "You’re verified!";

  static String get youhavebeensucces => "You have been succes fully verified";

  static String get done => "Done";

  static String get setupfaceid => "Setup Face ID";

  static String get unlockcrypto =>
      "Unlock Cryptoline with your face ID,\nquick and secured";

  static String get scanmyface => "Scan my face";

  static String get fingerprint => "Fingerprint";

  static String get unlockfinger =>
      "Unlock Cryptoline with your Fingerprint,\nquick and secured";

  static String get setupfingerprint => "Setup Fingerprint";

  static String get createnewpin => "Create New Pin";

  static String get addingapin =>
      "Adding a pin number will make your\ninvestment secure";

  static String get resetpass => "Reset password";

  static String get enteranemailadress =>
      "Enter an email address you use\nto sign in to";

  static String get letsverify => "Let’s verify your identity";

  static String get chooseyourdocument =>
      "Choose your document to verify your identity";

  static String get methodofverification => "METHOD OF VERIFICATION";

  static String get passport => "Passport";

  static String get identitycard => "Identity Card";

  static String get digitaldocument => "Digital Document";

  static String get verifyaccount => "Verify Account";

  static String get takeaphotooffront =>
      "Take a photo of front of your Identity Card";

  static String get termsofservice => "Terms of Service";

  static String get lastupdate => "Last update on August 2021";

  static String get terms => "Terms";

  static String get termslorem =>
      "Lorem ipsum dolor sit amet, consectetur adip ielit ut aliquam, purus sit amet luctus venenatis, lectus magna fringilla urna, porttitor rhoncus dolor purus non enim praesent  facilisis leo, vel fringilla  eget nulla facilisi etiam diam quis enim lobortis scelerisque fermentum dui faucibus in ornare quam viverra.";

  static String get uselicense => "Use License";

  static String get adipiscingtempus =>
      "Adipiscing tempus feugiat viverra iaculis modo quisque dictum quis tellus. Odio et a ac pretium nulla pharetra in. Cursus aenean condimentum volutpat ullamcorper eu, feugiat sed massa. In lacinia scelerisque a tortor aliquetEst et aenean proin facilisis sed aliquam vitae. Adipiscing porttitor lacus sit in. Dignissim entum blandit massa, est viverra volutpat sed tellus. Mauris purus habitant duis.";

  static String get declined => "Declined";

  static String get accept => "Accept";

  static String get gocrypto => "Монеточка";

  static String get buildingtrustinthecrypto => "Валютные операции";

  static String get favorites => "Favorites";

  static String get seeall => "See All";

  static String get btc => "BTC";

  static String get bitcoin => "Bitcoin";

  static String get eth => "ETH";

  static String get ethereum => "Ethereum";

  static String get bnb => "BNB";

  static String get binance => "Binance";

  static String get liveprices => "Live Prices";

  static String get litecoin => "Litecoin";

  static String get ltc => "LTC";

  static String get xrp => "XRP";

  static String get myPortfolio => "My Portfolio";

  static String get deposit => "Deposit";

  static String get withdraw => "Withdraw";

  static String get market => "Market";

  static String get marketcap => "Market Cap";

  static String get volume => "24th Volume";

  static String get btcdominance => "BTC Dominance";

  static String get ltcusd => "LTC/USD";

  static String get litecoinltc => "Litecoin/LTC";

  static String get description => "Description";

  static String get litecoinisa =>
      "Litecoin is a cryptocurrency launched in late 2011 by former Google and Coinbase engineer Charlie Lee. To create Litecoin, Lee copied the Bitcoin codebase, increased the total supply, and changed the speed at which new blocks are added to the blockchain.";

  static String get sell => "Sell";

  static String get buy => "Buy";

  static String get exchange => "Exchange";

  static String get yousend => "You Send";

  static String get youreceive => "You Receive";

  static String get dai => "DAI";

  static String get exchangefee => "Exchange fee";

  static String get clickherefor => "Click here for";

  static String get termsandcondition => "Terms & Conditions ";

  static String get forthistranjection =>
      "For this transaction fee will be taken";

  static String get exchangenow => "Exchange Now";

  static String get selectcrypto => "Select Crypto";

  static String get confirmation => "Confirmation";

  static String get to => "to";

  static String get amount => "Amount";

  static String get free => "Fee";

  static String get total => "Total";

  static String get processedbycryptoline =>
      "Processed by Cryptoline every purchases";

  static String get securedbythe => "secured by the ";

  static String get confirmorder => "Confirm Order";

  static String get success => "Success!";

  static String get hasbeenexchange => "Has been exchange from ETH to DAI";

  static String get youpay => "You Pay";

  static String get usd => "USD";

  static String get creditcard => "Credit Card";

  static String get googlepay => "Google Pay";

  static String get mobilebanking => "Mobile Banking";

  static String get sendreceipt => "Send receipt to your email";

  static String get orderpreview => "Order Preview";

  static String get balance => "Balance";

  static String get paymentmethod => "Payment Method";

  static String get purchaseprice => "Purchase Price";

  static String get profile => "Profile";

  static String get hirenjoshi => "Hiren Joshi";

  static String get general => "General";

  static String get myaccount => "My Account";

  static String get billingpayment => "Billing/Payment";

  static String get faqsupport => "FAQ & Support";

  static String get language => "Language";

  static String get darkmode => "Dark Mode";

  static String get fullname => "Full name";

  static String get editprofile => "Edit Profile";

  static String get email => "Email";

  static String get selectlanguage => "Select Language";

  static String get english => "English";

  static String get italy => "Italy";

  static String get japanese => "Japanese";

  static String get indonesian => "Indonesian";

  static String get russian => "Russian";

  static String get savechange => "Save Changes";

  static String get portofolio => "Portofolio";

  static String get notication => "Notification";

  static String get offers => "Offers";

  static String get gratisminyank =>
      "Gratis minyank 1L dan beras 5KG\ncuma beli isi ulang Aqua ajal";

  static String get zerominute => "0 minute ago";

  static String get paymentsucesses => "Payment Successful";

  static String get yousendpayment =>
      "You send a payment of \$10.00\$\nfor Cumi Asin Seuhah";

  static String get yesterday => "Yesterday";

  static String get ojek =>
      "You send a payment of \$10.00\$\nfor Ojek Pangkalan";

  static String get july => "24 july 2022";
static String get logout  => "Log Out" ;
// static String get   =>  ;
// static String get   =>  ;
// static String get   =>  ;
// static String get   =>  ;
// static String get   =>  ;
// static String get   =>  ;
// static String get   =>  ;
// static String get   =>  ;
// static String get   =>  ;
// static String get   =>  ;
// static String get   =>  ;

}
